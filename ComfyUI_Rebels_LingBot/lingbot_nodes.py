...
class LingBotSampler:
    ...
    def sample(self, lingbot_model, embeds, width, height, num_frames, steps,
        ...
        try:
            ...
            print("[LingBot] denoise done; evicting DiT and decoding ({} latent frames, tiled)".format(
                latents.shape[2] if hasattr(latents, "shape") else "?"))
            transformer.to("cpu")
            gc.collect()
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            vae.to(device)
            with torch.inference_mode():
                frames = pipe._decode_latents(latents.to(device))
            vae.to("cpu")
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            import numpy as np
