    ...
    def build(self, scene_description, camera_movement, subject_name,
        ...
        if subject_name.strip():
            payload["prominent_elements"].append(self._element(
                ...
                subject_action.strip(), duration_s,
                "Center foreground", "large",
        ...
        if environment_name.strip():
            payload["prominent_elements"].append(self._element(
                ...
                "", duration_s, "Background", "dominant",
        ...
        print("[LingBot] structured caption: {} chars, {} element(s), "
              "lighting={}/{}, camera_movement={}".format(
                  len(out), len(payload["prominent_elements"]),
                  lighting_type, lighting,
                  "yes" if camera_movement.strip() else "no"))
        return (out,)
...
class _GGUFExpertStack:
    """Replaces experts.w1/w2/w3 params. Indexing dequants ONE expert's 2D
    matrix on demand onto the compute device - matches the vendored
    _run_experts_for_loop access pattern (w[idx].transpose(-2, -1))."""

    def __init__(self, per_expert, device, dtype):
        # per_expert: list of (qdata_cpu, qtype, oshape)
        self._e = per_expert
        self.device = device
        self.dtype = dtype
...
class LingBotMoELoaderGGUF:
    ...
    def load(self, dit_gguf, vae_name, device):
        ...
        os.environ["LINGBOT_MOE_EXPERT_BACKEND"] = "for_loop"
        ...
        experts = defaultdict(dict)  # "blocks.N.ffn.experts.w1" -> {e: (q, t, shape)}
        attached, unmatched = 0, []
        import re
        exp_re = re.compile(r"^(.*\.experts\.w[123])\.(\d+)$")
        ...
        # install expert stacks: replace the fused meta Parameters with our
        # indexable dequant-on-demand objects
        n_stacks = 0
        for full, emap in experts.items():
            ...
            object.__setattr__(parent, wname, _GGUFExpertStack(ordered, dev, torch.bfloat16))
            n_stacks += 1
